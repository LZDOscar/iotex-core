...
func (bc *blockchain) MintNewBlock(
	...
	//blk := block.NewBlock(bc.config.Chain.ID, bc.tipHeight+1, bc.tipHash, bc.now(), producer.PublicKey, actions)
	blkbd := block.NewBuilder().
		SetChainID(bc.config.Chain.ID).
		SetHeight(bc.tipHeight + 1).
		SetPrevBlockHash(bc.tipHash).SetTimeStamp(bc.now()).
		AddActions(actions...)
...
func (bc *blockchain) MintNewSecretBlock(
	...
	blk, err := blkbd.SetReceipts(receipts).
		SetStateRoot(root).
		SignAndBuild(producer)
	if err != nil {
		return nil, errors.Wrapf(err, "Failed to create block")
...
func (bc *blockchain) startEmptyBlockchain() error {
	...
	if bc.config.Chain.GenesisActionsPath != "" || !bc.config.Chain.EmptyGenesis {
		...
		genesisbd := NewGenesisBlock(bc.config.Chain, ws)
		if genesisbd == nil {
		...
		genesis, err = genesisbd.SetReceipts(receipts).
			SetStateRoot(root).
			SignAndBuild(iaddr)
		if err != nil {
